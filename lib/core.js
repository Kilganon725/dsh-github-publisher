...
import { existsSync, mkdirSync, readFileSync, writeFileSync, rmSync, mkdtempSync, copyFileSync } from 'node:fs'
...
export async function publish(opts) {
  ...
  if (existsSync(inst)) {
    // gh 对非 ASCII 文件名会回退成 default.txt；复制为 ASCII 名再上传，保证附件名可读
    try {
      const instAscii = join(dest, 'install-guide.txt')
      copyFileSync(inst, instAscii)
      assets.push(instAscii)
    } catch {
      assets.push(inst)
    }
  }
